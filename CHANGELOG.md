...
- Digital Miner skill requirement enforcement now works correctly when `enableDigitalMinerXP` is disabled
- Previously, disabling XP would cause the early return to skip skill requirement checks entirely
- Now only returns early if BOTH `enableDigitalMinerXP` and `enableDigitalMinerSkillRequirements` are disabled
